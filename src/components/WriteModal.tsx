...
export const writeFormSchema = z.object({
    title: z.string().min(10).max(38),
    description: z.string().min(70).max(100),
...
const WriteModal: NextPage= (({}) => {
    const { setIsWriteModel } = useContext(GlobalContext)
    ...
    const createPost = api.post.writePost.useMutation({
        onSuccess: () => {
    ...
    useClickOutside(modalRef, () => setIsWriteModel(false))
...
<div  className='absolute inset-0 bg-black/50 dark:bg-white/20 h-full w-full flex justify-center items-center px-4 z-50'>
    ...
    <div ref={modalRef} className='aspect-square  w-full max-w-[500px] max-h-[900px]  dark:bg-darkBg bg-lightBg rounded-md shadow-md flex justify-start items-center flex-col p-4 relative' >
        <div className='absolute top-4 right-4 cursor-pointer'><Button><X onClick={() => setIsWriteModel(false)} size='32px' /></Button></div>
        <h2 className='font-[500] text-2xl pt-2'>Make a New Post</h2>
        ...
        <form onSubmit={handleSubmit(onSubmit) } className='flex justify-center items-center flex-col space-y-3 w-full py-4 px-6 md:p '>
            ...
            Title is Required, 10-30 characters.
            </p>}
            ...
            Description is Required, 70-100 characters .
            </p>}
